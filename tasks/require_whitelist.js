        ...
        if (require.allowed) {
          if(require.reason !== "local" && require.reason !== "whitelisted" ){
            grunt.log.writeln(
              require.file +
              " requires " +
              require.requireString["grey"] +
              ": line " +
              require.line +
              " " +
              require.reason["green"]
            );
          }
        } else {
          grunt.log.writeln(
            require.file +
              " requires " +
              require.requireString["red"].bold +
              ": line " +
              require.line +
              " " +
              require.reason["red"]
          ...
          if (!grunt.file.isDir(filename)) {
            ...
            if (requires) {
              for (var i = 0; i < requires.nodes.length; i++) {
                var temp = {
                  file: filepath,
                  requireString: requires.strings[i],
                  line: requires.nodes[i].loc.start.line
                };
        ...
        function(err) {
          ...
          if (err) {
            ...
            if (errorRequires.length > 0) {
              grunt.log.error(
                "The following require statements are not allowed:"
              );
